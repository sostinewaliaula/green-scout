...
export type CmsBlock = BlockText | BlockImage | BlockGallery | BlockStats | BlockTestimonial;
      ...
      {content?.map((block: CmsBlock, idx: number) => {
        switch (block._type) {
          ...
          case 'blockStats': {
            ...
            return (
              <section key={idx} className="max-w-6xl mx-auto px-4">
                {b.title && <h3 className="text-2xl font-semibold text-green-700 mb-6">{b.title}</h3>}
