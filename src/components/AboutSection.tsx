    ...
    <section id="about" className="py-20 px-4 md:px-8 bg-green-100">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center">
          <div className="md:w-1/2 transform transition-all duration-700 hover:scale-[1.02]">
            <img
          ...
          <div className="md:w-1/2 space-y-4">
            {introText ? (
              introText.split('\n\n').map((p, i) => (
                <p key={i} className="text-lg text-gray-700">{p}</p>
              ))
            ) : (
              <>
            ...
            <div className="pt-4">
              <div className="flex flex-wrap gap-4">
                {(cms?.stats && cms.stats.length > 0 ? cms.stats : [
                  ...
                  <div key={i} className={`rounded-lg p-4 flex items-center gap-3 flex-1 min-w-[200px] hover:shadow-md transition-shadow ${s.theme === 'purple' ? 'bg-purple-200' : 'bg-green-200'}`}>
                    <div className={`${s.theme === 'purple' ? 'bg-purple-600' : 'bg-green-600'} w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-xl transform transition-transform hover:scale-110`}>
                      {s.value}
                    </div>
                    <div>
